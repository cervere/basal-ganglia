...
cogstim=np.array([[2, 3]])
motstim=np.array([[1, 0]])
...
#np.array([ 0.395, 0.465, 0.535, 0.625])*1
    nengo.Connection(CTX_COG_out, STR_COG.input, transform=np.array([ 0.49486137,  0.49597046,  0.53122533,  0.56325616]), synapse = tau)
    nengo.Connection(CTX_MOT_out, STR_MOT.input, transform=weights(inps), synapse = tau)
    nengo.Connection(CTX_ASC_out, STR_ASC.input, transform=weights(inps*inps), synapse = tau)
...
#    stim_asc.output = PresentInput(inputs=np.hstack([fill(cogstim), fill(motstim)]), presentation_time=0.5)
    ...
    def ascstim(asc):
        ...
        if (cog[0] + cog[1]) == 0 or (mot[0] + mot[1]) == 0: # Trial reset
    ...
    stim_asc = nengo.Ensemble(40,4, neuron_type=nengo.Direct())
    nengo.Connection(stim_cog, stim_asc[0:2])
    nengo.Connection(stim_mot, stim_asc[-2:])
...
print(np.hstack([fill(cogstim), fill(motstim)]))
